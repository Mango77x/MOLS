...
- [ ] Backend: add `spring-boot-starter-mail` — no email dependency exists in `pom.xml` today. New `MailProperties`-style config under `mols.mail.*` (host/port/username/password/from-address), following the existing `@ConfigurationProperties` pattern (`DashboardProperties`, `JwtProperties`, `LockoutProperties`). Default to disabled unless SMTP env vars are present — the app must keep booting cleanly on a machine with no mail server configured, mirroring `BootstrapAdminProperties`'s opt-in pattern.
- [ ] A `@Scheduled` job (daily cron) reusing `DashboardService`'s existing `lowStockAlerts()`/`staleOrderAlerts()` queries (`DashboardService.java:117-133`) rather than duplicating that logic — the job composes the same alert lists the Dashboard already renders into an email instead of a UI panel. Needs a recipient list; simplest v1 is every enabled ADMIN's email, which requires adding a nullable `email` column to `app_users` (new Flyway migration — existing users have none) plus an editable field on `UserFormPage`/`UsersPage`.
- [ ] Self-service password reset email: `AppUserAdminService.resetPassword(id, rawPassword)` today is admin-driven only (an ADMIN sets a new password directly) — there is no user-initiated flow at all, so this is new work, not a channel swap on an existing one. Needs: `POST /api/auth/forgot-password` (email lookup, always answers 200 regardless of whether the email exists, to avoid account enumeration), a signed/expiring reset token (reuse the JWT infrastructure already in `JwtService`, with a distinct claim so `JwtAuthFilter` can't mistake it for a session token), `POST /api/auth/reset-password?token=`, and a frontend `ResetPasswordPage`. Replaces Sprint 15's static "contact your administrator" `LoginPage` hint for users who *have* an email on file; keep the static hint as the fallback for accounts without one.
- [ ] Trigger points: low stock crossing `mols.dashboard.low-stock-threshold`, an order stale past `mols.dashboard.stale-order-days` (both existing `DashboardProperties`), and the password-reset flow above.
- [ ] Scope check before starting: confirm with the maintainer whether alert emails need to be per-user configurable (which alerts, how often) or a single fixed daily digest for v1 — default to the fixed policy first, add configurability only if requested (consistent with `mols.dashboard.*` already being one app-wide setting, not per-user).
- [ ] Tests: unit-test the scheduled job's alert-selection logic against `DashboardService`'s existing fixtures, mocking `JavaMailSender` (no real SMTP call in unit tests). The actual send path needs either an integration test or a manual `docker compose` check against a local SMTP catcher (e.g. Mailpit) — Testcontainers-based integration tests don't run on this machine (see Verification section below).
- [ ] Docs: `PROJECT_OVERVIEW.md`; `HELP.md` (the new SMTP env vars an operator sets to enable this); consider adding an optional local mail-catcher service to `docker-compose.yml` for dev so the send path is testable without a real SMTP account.
