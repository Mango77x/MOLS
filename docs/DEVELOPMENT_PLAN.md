...
- [x] **"Recent activity" shows raw stock IDs instead of names.** `RecentActivity.tsx:49` rendered `#{movement.stockId}` directly. The dashboard's own "Low stock" alerts panel already solved this same problem correctly — `DashboardResponse.LowStockAlert` (backend, `DashboardService.java:117-124`) carries `resourceName`/`warehouseName` fields precomputed server-side — but `RecentMovement` (`pages/dashboard/types.ts:47-57`) only has `stockId`, and the backend's `MovementResponse` never carries names either. Rather than changing the API contract, copied `/app/movements`'s (`MovementsPage.tsx:29-31,39-48`) existing client-side fix: three `useLookup` calls (`/stocks`, `/resources`, `/warehouses`) plus `resourceName(stockId)`/`warehouseName(stockId)` helpers, added straight into `RecentActivity.tsx` (not `DashboardPage.tsx` — keeps the lookups scoped to the one component that needs them instead of prop-drilling). The single "Stock" column became two: "Resource" and "Warehouse", matching `MovementsPage`'s own column split.
   - Test: `RecentActivity.test.tsx` (new) mocks the three lookups and asserts the resolved names render instead of `#id`, plus a fallback-placeholder case (stock id with no matching lookup row) and the pre-existing empty-state case.
- [x] **Static legend on the donut charts.** `DonutChart` (`pages/dashboard/Charts.tsx:72-100`), used for both "Movements by type" and "Orders by status", only identified slice colors via the Recharts `<Tooltip>` on hover — no way to read them without a mouse, or on touch. Added a custom `DonutLegend` component underneath each chart (not Recharts' own `<Legend>`, to match the rest of the app's styling): a row of `{color swatch, label, value}` per slice, reusing `toRows(series)`/`STATUS_COLORS`/`FALLBACK_COLORS` directly so the legend and the slices can never disagree on color.
   - Test: `Charts.test.tsx` (new) asserts each label/value pair renders in the legend for both donut charts, and that an empty series renders no legend (the existing empty-state chart takes over instead).
...
**Last updated**: 2026-07-21 (Sprint 13 complete)
