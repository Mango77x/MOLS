...
1. **Create/edit forms give zero positive feedback on success — re-checked against `main` as of Sprint 11, still true everywhere.** All 9 form pages (`WarehouseFormPage`, `ResourceFormPage`, `UnitFormPage`, `VehicleFormPage`, `StockCreateFormPage`, `StockAdjustFormPage`, `OrderEditFormPage`, `ShipmentFormPage`, `UserFormPage`) already use `FormBanner`/`FieldError` *consistently* for validation and API errors (confirmed by grep — the original "audit forms for consistent usage" framing undersold how done this already is). The actual gap is different: `FormBanner` (`components/form/fields.tsx:18-28`) is hardcoded to the `status-critical` error styling — there's no success tone — and every form's `onSubmit` just calls `navigate(...)` back to the list on success with no toast, no banner, nothing (see `WarehouseFormPage.tsx:74-84` for the representative pattern, identical elsewhere). Meanwhile `RowActions`'s delete flow (Sprint 10/11) *does* show a toast on both success and failure via `useToast()` (`components/toast/toastContext.ts`), so deletes now have better feedback than creates/edits — an inconsistency introduced by Sprint 10, not present before it.
   - Fix: call `showToast('Warehouse created.')` / `'Warehouse updated.'` (etc., one string per entity/verb) right before each `navigate(...)` in all 9 form pages, reusing the existing `ToastProvider`/`useToast` from `components/toast/` — no new component needed.
   - Test: extend the existing per-page test files (`UserFormPage.test.tsx` etc., or add analogous ones for pages that don't have one yet) to assert the toast text appears — reuse the `ToastProvider` wrapper pattern already established in `RowActions.test.tsx`.
2. **Card view for data tables on mobile viewports.** `DataTable.tsx:49` wraps the `<table>` in `overflow-x-auto` — on a narrow viewport this just becomes a horizontally-scrolling table with the actions column scrolled out of view by default, easy to miss. Add a `sm:hidden` card-list rendering path alongside the existing `hidden sm:block` (or similar breakpoint) table, driven by the same `columns`/`data` props `DataTable` already receives — the `ColumnDef` array already carries `header`/`cell` per column, so the card view can iterate the same `table.getRowModel().rows` and render each cell as a labeled row instead of a `<td>`, without touching any call site (`WarehousesPage`, `ResourcesPage`, `VehiclesPage`, `OrdersPage`, `ShipmentsPage`, `StocksPage`, `UnitsPage`, `UsersPage`, `MovementsPage` all go through this one component).
   - Test: a `DataTable.test.tsx` rendering at a mobile viewport width (jsdom doesn't do real layout, so this means asserting the card markup is present/table markup is `aria-hidden` or absent, not visual — check how other tests in this repo handle Tailwind breakpoints, since this is the first one that needs to).
