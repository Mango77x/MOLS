...
**Pagination (opt-in)**: every list endpoint accepts optional `page`, `size`
(1–100, default 20) and `sort` (`field` or `field,desc`; fields are whitelisted
per entity) query parameters. Without them the plain JSON array is returned
(original contract). With any of them the response becomes a stable envelope:
...
pagination or sort parameters return HTTP 400.
